...
class _SolicitarObjetoPageState extends State<SolicitarObjetoPage> {
  ...
  void initState() {
    ...
    _origem = draft.origemDescricao.isEmpty ? null : draft.origemDescricao;
    _destino = draft.destinoDescricao.isEmpty ? null : draft.destinoDescricao;
    _data = draft.data.isEmpty ? null : draft.data;
    _horario = draft.horario.isEmpty ? null : draft.horario;
    _horarioController.text = _horario ?? '';
    _dataSelecionada = _parseData(draft.data);
    _horarioErro = _validarHorario(_horarioController.text, _dataSelecionada);
  ...
  void _aoAlterarHorario(String value) {
    setState(() {
      _horario = value.isEmpty ? null : value;
  ...
  String? _validarHorario(String value, DateTime? data) {
    ...
    if (!dataHoraSolicitacaoValida(dataBase, horario)) {
      return 'A data e o horário do transporte devem estar no futuro.';
    }
  ...
  void _mostrarErroDataHora() {
    ScaffoldMessenger.of(context)
      ...
      ..showSnackBar(
        const SnackBar(
          content: Text(
            'A data e o horário do transporte devem estar no futuro.',
          ),
        ),
      );
  }
  ...
  void _avancar() {
    ...
    final draft = bloc.state.rascunho.copyWith(
      ...
      origemDescricao: _origem!,
      origemPoint: _origemPoint,
      destinoDescricao: _destino!,
